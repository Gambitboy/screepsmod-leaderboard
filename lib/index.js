...
module.exports = function(config) {
    ...
    config.backend.router.get('/leaderboard', async (request, response) => {
        ...
        const filtered = data.filter(item => item.username && !IGNORE_LIST.includes(item.username));
        const sorted = filtered.sort((a, b) => b.gcl - a.gcl);
        const remap = sorted.map(({ username, gcl, badge }, index) => ({ position: index + 1, username, gcl: gcl.toLocaleString(), badge }))
